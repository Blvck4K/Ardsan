    ...
    document.getElementById('userForm')?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const { error } = await supabase.from('users').update({
                username, phone, next_of_kin: nok, address, ippis_number: ippis, member_id, is_admin: isAdmin,
...
async function loadUsers() {
    ...
    try {
        const { data: users, error } = await supabase
            .from('users')
            .select('id, email, username, phone, next_of_kin, address, ippis_number, member_id, balance, shares, contributions, status, is_admin');

        if (error) throw error;

        // Fetch ippis records to map account_number
        let ippisMap = {};
        try {
            const { data: ippisData, error: ippisError } = await supabase.from('ippis_records').select('ippis_number, account_number');
            if (ippisData) {
                ippisData.forEach(r => ippisMap[r.ippis_number] = r.account_number);
            }
        } catch (e) { console.warn("Could not fetch ippis records", e); }
        ...
        filteredUsers.forEach((u, idx) => {
            ...
            let accNo = 'N/A';
            if (u.ippis_number && ippisMap[u.ippis_number]) {
                accNo = ippisMap[u.ippis_number];
            }
            html += `
                <tr>
                    <td>
...
window.openEditUserModalByIdx = function (idx) {
    ...
    document.getElementById('editUserSavings').value = u.savings || 0;
    document.getElementById('editUserShares').value = u.shares || 0;
    document.getElementById('editUserContributions').value = u.contributions || 0;
    ...
    users.forEach((u) => {
        html += `
            <tr>
                ...
                <td>
                    <input type="number" id="contributions_${u.id}" class="inline-input" style="width: 120px; padding: 5px; border-radius: 4px; border: 1px solid #ccc;" value="${u.contributions || 0}" step="0.01">
                </td>
                <td>
                    <button class="btn-small btn-approve" onclick="updateUserContribution('${u.email}', 'contributions_${u.id}')"><i class="fa-solid fa-save"></i> Save</button>
                </td>
            </tr>
        `;
    });
...
window.updateUserContribution = async function (email, inputId) {
    ...
    if (!confirm(`Are you sure you want to update the contribution balance for ${email} to ₦${newAmount.toLocaleString()}?`)) return;

    try {
        const { error } = await supabase
            .from('users')
            .update({ contributions: newAmount })
            .eq('email', email);

        if (error) throw error;

        alert(`Successfully updated contributions for ${email}.`);
        // Refresh the user cache and both tables
        await loadUsers();
        loadContributionsList();
    } catch (err) {
    ...
    users.forEach((u) => {
        html += `
            <tr>
                ...
                <td>
                    <input type="number" id="savings_${u.id}" class="inline-input" style="width: 120px; padding: 5px; border-radius: 4px; border: 1px solid #ccc;" value="${u.savings || 0}" step="0.01">
                </td>
                <td>
                    <button class="btn-small btn-approve" onclick="updateUserSavings('${u.email}', 'savings_${u.id}')"><i class="fa-solid fa-save"></i> Save</button>
                </td>
            </tr>
        `;
    });
...
window.updateUserSavings = async function (email, inputId) {
    const inputEl = document.getElementById(inputId);
    if (!inputEl) return;
    ...
    if (!confirm(`Are you sure you want to update the savings balance for ${email} to ₦${newAmount.toLocaleString()}?`)) return;

    try {
        const { error } = await supabase
            .from('users')
            .update({ savings: newAmount })
            .eq('email', email);

        if (error) throw error;

        alert(`Successfully updated savings for ${email}.`);
        await loadUsers();
    } catch (err) {
        alert('Error updating savings: ' + err.message);
        console.error(err);
    }
}
